...
dotnet run --project src/Cli -- review `
  --doc      "C:\caminho\para\seu\documento.docx" `
  --profile  "profiles\exemplo.json" `
  --out      "C:\caminho\para\saida\documento-revisado.docx"
```
...
- Os **comentários** ficam visíveis no painel lateral de revisão (`Revisão →
  Mostrar Comentários`).
